    ...
    for dir_entry in std::fs::read_dir(input)? {
        ...
        match input_file_kind {
            FileKind::Map => {
                ...
                let map: rpgmz_types::Map = serde_json::from_str(&input_str)
                ...
                for (event_id, event) in map.events.iter().enumerate() {
                    ...
                    for (page_index, page) in event.pages.iter().enumerate() {
                        ...
                        .with_context(|| {
                            format!("failed to convert map {map_id}, event {event_id}")
                        })?;
                    }
                }
            }
            FileKind::CommonEvents => {
                ...
                let common_events: Vec<Option<rpgmz_types::CommonEvent>> =
                ...
                for (common_event_id, common_event) in common_events.iter().enumerate() {
                    ...
                    .context("failed to convert common events file")?;
                }
            }
            FileKind::Troops => {
                output.push("troops");

                let troops: Vec<Option<rpgmz_types::Troop>> = serde_json::from_str(&input_str)
                    .with_context(|| format!("failed to parse \"{}\"", input.display()))?;
                ...
                for (troop_id, troop) in troops.iter().enumerate() {
                    ...
                    for (page_index, page) in troop.pages.iter().enumerate() {
                        ...
                        .context("failed to convert troops file")?;
                    }
                }
            }
            FileKind::Dir => {
...
fn dump_file(
    ...
    let event_commands = match input_file_kind {
        FileKind::Map => {
            let mut map: rpgmz_types::Map = serde_json::from_str(&input_str)
        ...
        FileKind::CommonEvents => {
            let mut common_events: Vec<Option<rpgmz_types::CommonEvent>> =
                serde_json::from_str(&input_str)
                    .with_context(|| format!("failed to parse \"{}\"", options.input.display()))?;
        ...
        FileKind::Troops => {
            let mut troops: Vec<Option<rpgmz_types::Troop>> = serde_json::from_str(&input_str)
                .with_context(|| format!("failed to parse \"{}\"", options.input.display()))?;
